...
- **Dispositivo sin sensor de podómetro → bloquea el inicio del paseo**, con una explicación de que el dispositivo no es compatible (decisión revisada 2026-07-26, ver §3). No hay ningún ajuste que abrir para corregirlo -- a diferencia del caso de permiso denegado, este mensaje no ofrece un botón de "Abrir ajustes".
  ...
  - **El emulador de Android no tiene este sensor** (confirmado en la documentación oficial de Extended Controls) -- esta función nunca se puede probar ahí, solo en un dispositivo real. Para poder seguir probando el resto del flujo del paseo en el emulador sin ese sensor, hay un toggle de debug ("Omitir requisito de podómetro", `DebugSettingsScreen`, agregado 2026-07-26) que salta este chequeo -- gateado por `kDebugMode` además de la ruta de debug ya inalcanzable en release, así que no existe forma de activarlo fuera de un build de debug.
