...
- **Tracking continuo de ubicación durante el paseo**: hoy la app solo geolocaliza **una vez**, al iniciar el paseo, para centrar el mapa (`paseo_screen.dart`). Detectar proximidad a un POI mientras se camina requiere agregar **tracking continuo de ubicación durante un paseo activo** — esto no existe todavía y es un cambio de arquitectura no trivial (consumo de batería, permisos de ubicación en segundo plano, etc.). Vale la pena discutirlo antes de estimar este spec como "chico".
