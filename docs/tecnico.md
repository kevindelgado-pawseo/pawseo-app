...
- **API key de Google Maps (Maps SDK for Android)**: `--dart-define` no aplica acá porque el `AndroidManifest.xml` se resuelve en build nativo, antes de que corra Dart. Se inyecta vía `android/local.properties` (gitignored, por máquina) → `manifestPlaceholders` en `android/app/build.gradle.kts` → `${googleMapsApiKey}` en el manifest. Cada dev necesita su propia key restringida a `cl.pawseo.app`/`cl.pawseo.app.dev` en Google Cloud Console (requiere billing habilitado en el proyecto de GCP, aunque el uso del SDK de mapas móvil en sí sea gratis e ilimitado).
